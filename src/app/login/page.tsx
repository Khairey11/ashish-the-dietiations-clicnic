...
function LoginForm() {
  ...
  // Surface OAuth error when bounced back from /api/auth/google/callback
  React.useEffect(() => {
    const err = searchParams.get("error");
    const reason = searchParams.get("reason");
    if (err === "google_auth_failed") {
      const map: Record<string, string> = {
        state_mismatch: "Security check failed. Please try Google sign-in again.",
        missing_parameters: "The Google sign-in response was incomplete. Please try again.",
        no_access_token: "Google did not return an access token. Please try again.",
        no_email: "We could not retrieve your email from Google. Please use email/password instead.",
        email_not_verified: "Your Google email is not verified. Please verify it and try again.",
        account_disabled: "This account has been disabled. Contact the clinic to re-enable it.",
        unknown_error: "Google sign-in failed. Please try again or use email/password.",
      };
      toast.error("Google sign-in failed", {
        description: (reason && map[reason]) || map.unknown_error,
      });
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              <a
                ...
                className="inline-flex items-center justify-center h-11 rounded-md border border-input bg-background hover:bg-accent hover:text-accent-foreground text-sm font-medium transition-colors"
